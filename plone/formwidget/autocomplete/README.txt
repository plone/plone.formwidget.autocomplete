...
z3c.formwidget.query, which we extend.
    ...
    ...         SimpleTerm('Torino', 'torino', 'Torino')))
    ...
...
Then, we will set up a simple test form and context.
    ...
    ...     favourite_city = schema.Choice(title="Favourite city",
    ...                                    source=ItalianCitiesSourceBinder())
    ...     visited_cities = schema.List(title="Visited cities",
    ...                                  value_type=schema.Choice(title="Selection",
    ...                                                           source=ItalianCitiesSourceBinder()))
...
Simulates traversal:
    ...
    >>> form_view = getMultiAdapter((context, request), name="cities-form")
...
Simulates partial rendering:
    ...
    ... $('#form-widgets-favourite_city-widgets-query').autocomplete('http://nohost/bar/@@cities-form/++widget++form.widgets.favourite_city/@@autocomplete-search', {
    ...

    >>> print(form.widgets['visited_cities'].render()) # doctest: +NORMALIZE_WHITESPACE +ELLIPSIS
    <script type="text/javascript"
    ... $('#form-widgets-visited_cities-widgets-query').autocomplete('http://nohost/bar/@@cities-form/++widget++form.widgets.visited_cities/@@autocomplete-search', {
    ...
...
like this:
    ...
    >>> search_view = getMultiAdapter((widget, query_request), name='autocomplete-search')
    >>> print(search_view())
    sorrento|Sorrento
    torino|Torino
...
z3c.formwidget.query extract() method.
  ...
  - If the search button from the query subform was clicked, then assume
    ...
    >>> form_view = getMultiAdapter((context, search_request), name="cities-form")
  ...
  - Otherwise, if the widget id and form submit button are in the request,
    ...
    >>> form_view = getMultiAdapter((context, search_request), name="cities-form")
    >>> form_view.form_instance.update()
    Submitted data: {'visited_cities': ['Torino']}
    >>> form_view.form_instance.widgets['visited_cities'].extract()
    ('torino',)
